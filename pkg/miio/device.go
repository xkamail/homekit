...
func (d *XiaomiDevice) getFieldValue(field fldName) string {
	...
	case reflect.String:
		return v.(string)
	default:
		slog.Warn("Unknown %s value type %s", field, reflect.TypeOf(v).Kind().String())
		return ""
	}
}
...
func (d *XiaomiDevice) GetFieldValueInt32(field fldName, curVal int32) int32 {
	...
	if err != nil {
		slog.Warn("Failed to parse int: %s", v)
		return curVal
	}
	return int32(n)
}
...
func (d *XiaomiDevice) GetFieldValueUint32(field fldName, curVal uint32) uint32 {
	...
	if err != nil {
		slog.Warn("Failed to parse uint32: %s", v)
		return curVal
	}
	return uint32(n)
}
...
func (d *XiaomiDevice) GetFieldValueFloat64(field fldName, curVal float64) float64 {
	...
	if err != nil {
		slog.Warn("Failed to parse float64: %s", v)
		return curVal
	}
	return n
}
...
func (d *XiaomiDevice) doCommand(cmd string, data []interface{}, storeResponse bool) bool {
	...
	if err != nil {
		slog.Error("Failed to marshal %s command: %s", cmd, err.Error())
		return false
	}

	p, err := d.crypto.NewPacket(b)
	if err != nil {
		slog.Error("Failed to encrypt %s command: %s", cmd, err.Error())
		return false
	}
...
func (d *XiaomiDevice) discovery() bool {
	...
	for {
		...
		case b := <-d.conn.DeviceMessages:
			if 32 != len(b) {
				slog.Error("Received incorrect discovery package")
				return false
			}

			p, err := packet.Decode(b, nil)
			if err != nil {
				slog.Error("Failed to decode packet", "error", err)
				return false
			}
		...
		case <-time.After(5 * time.Second):
			slog.Error("Timeout while waiting on handshake")
			return false
		}
	}
...
func (d *XiaomiDevice) sendAndWait(p *packet.Packet, cmd string, storeResponse bool) bool {
	...
	for {
		...
		case b := <-d.conn.DeviceMessages:
			...
			if err != nil {
				slog.Error("Failed to verify packet", "error", err)
				continue
			}

			dec, err := d.crypto.Decrypt(p.Data)
			if err != nil {
				slog.Error("Failed to decrypt packet", "error", err)
				continue
			}
			...
			if err != nil {
				slog.Error("Failed to un-marshal response", "error", err)
				continue
			}
